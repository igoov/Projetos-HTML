...
const fishData = [
    ...
    // Covos (simplificado para os locais principais)
...
function renderFish() {
    ...
    // Adicionar listeners após a renderização
...
function showMapForFish(fishName, mapKey) {
    ...
    // Ajusta o tamanho do canvas para o tamanho da imagem renderizada
    mapCanvas.width = image.clientWidth;
    mapCanvas.height = image.clientHeight;

    ctx.clearRect(0, 0, mapCanvas.width, mapCanvas.height); // Limpa o canvas
    
    const locations = mapCoordinates[mapKey];
    if (!locations) {
        console.error(`Chave de mapa não encontrada: ${mapKey}`);
        return;
    }

    ctx.fillStyle = 'rgba(255, 235, 59, 0.5)'; // Amarelo translúcido para o preenchimento
    ctx.strokeStyle = 'rgba(255, 100, 0, 0.9)'; // Laranja para a borda
    ctx.lineWidth = 3;
...
function toggleFishCaught(fishName, isCaught) {
    ...
    updateProgress();
    // Re-renderiza apenas o card afetado para melhor performance
